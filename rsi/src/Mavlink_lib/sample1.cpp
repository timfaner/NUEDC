...
protected:
	...
	bool data_request(uint8_t interval1,uint8_t interval2,uint8_t interval3);
	bool home_position();
...
bool Mavlink::data_request(uint8_t interval1,uint8_t interval2,uint8_t interval3)
{
	bool res=1;
	delay(200);
	mavlink_msg_request_data_stream_pack(GCS_ID,0,&sendmsg,0,1,0,1000/interval1,1);
	uint16_t valid_lenth=cut();
	res&=Serialwrite(senddata,valid_lenth);
	//Set important message interval [in microseconds]
	delay(200);
	mavlink_msg_command_long_pack(GCS_ID,0,&sendmsg,1,1,MAV_CMD_SET_MESSAGE_INTERVAL,0,30,interval2,0,0,0,0,0);//attitude
	valid_lenth=cut();
	res&=Serialwrite(senddata,valid_lenth);
	delay(200);
	mavlink_msg_command_long_pack(GCS_ID,0,&sendmsg,1,1,MAV_CMD_SET_MESSAGE_INTERVAL,0,32,interval3,0,0,0,0,0);//local position
	valid_lenth=cut();
	res&=Serialwrite(senddata,valid_lenth);
	return res;
}
...
bool Mavlink::Receive(unsigned char a)
{
	if(mavlink_parse_char(MAVLINK_COMM_0,a,&recvdata,&recvstatus))
	{
		...
		case 253:
			...
			break;
		case 100:
			mavlink_msg_optical_flow_decode(&recvdata,&opt);
...
public:
	...
	bool emergency_stop();
	bool receive(unsigned char a);
	bool requestDataStream(uint8_t interval1,uint8_t interval2,uint8_t interval3);
...
{
	set_point_vel(localpoint.x,localpoint.y,localpoint.z);
	delay(1000);
...
bool Mavcontrol::requestDataStream(uint8_t interval1,uint8_t interval2,uint8_t interval3){
	if(!data_request(interval1,interval2,interval3))
	{
		debug_text("request data failed\n");
		return false;
	}
	...
	{
		debug_text("Request Data Frequency Succeed\n");
		return true;
	}
}
